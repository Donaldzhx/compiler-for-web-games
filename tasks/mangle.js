...
class Mangle extends Task {
    ...
    execute(input) {
        ...
        const mangleNames = analyze(input, this.config.force || [], this.config.skip || []);

        //  Stripping the comments and the strings to avoid inexistent confilcts
        ...
        const lines = inputWithoutStrings.split('\n');

        const mangleMap = {};
        let mangleIndex = 0;
        mangleNames.forEach((name) => {
            while(true){
                const mangled = encodeNumber(mangleIndex++);

                // Check if the mangled name is already int the original input
